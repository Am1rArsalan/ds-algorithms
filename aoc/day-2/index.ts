...
function passwordParser(
  pass: string
): { min: number; max: number; C: string; L: number } {
  let min: number = -1;
  let max: number = -1;

  // #-# c: password
  let dashIdx = pass.indexOf("-");
  min = parseInt(pass.substr(0, dashIdx));

  let spaceIdx = pass.indexOf(" ");
  max = parseInt(pass.substr(++dashIdx, spaceIdx));

  let colonIdx = pass.indexOf(":");
  let C = pass[--colonIdx];
  ...
  return {
    ...
    max,
    C,
    L: (pass.match(new RegExp(C, "g")) || []).length - 1,
  };
}

function findValidPasswords(): number {
  let R: number = 0;
  for (let i = 0; i < policies.length; ++i) {
    let item = policies[i];
    const { min, max, C, L } = passwordParser(item);
    if (min <= L && L <= max) {
      ++R;
    }
  }
...
function passwordParserP2(
  pass: string
): { P1: number; P2: number; C: string; checkLen: number } {
  let P1: number = -1;
  let P2: number = -1;

  // #-# c: password
  let dashIdx = pass.indexOf("-");
  P1 = parseInt(pass.substr(0, dashIdx));

  let spaceIdx = pass.indexOf(" ");
  P2 = parseInt(pass.substr(++dashIdx, spaceIdx));

  let colonIdx = pass.indexOf(":");
  let C = pass[--colonIdx];

  let passPhrase = pass.substr(colonIdx + 3);
  let checkLen = 0;
  passPhrase[P1 - 1] === C && ++checkLen;
  passPhrase[P2 - 1] === C && ++checkLen;
...
function findValidPasswordsP2(): number {
  let R: number = 0;
  for (let i = 0; i < policies.length; ++i) {
    let item = policies[i];
    const { checkLen } = passwordParserP2(item);
    if (checkLen === 1) {
      ++R;
    }
  }
...
console.log(findValidPasswordsP2());
